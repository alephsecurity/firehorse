...
class Framework:
    ...
    @staticmethod
    def peek(addr, size):
        d = Framework.get_peek_xml()\
                                .replace("ADDR", hex(addr)\
                                .replace("L", ""))\
                                .replace("SIZE", hex(size).replace("L", ""))
    ...
    def poke(addr, size, value, print_data=False):
        ...
        d = Framework.get_poke_xml()\
                                    .replace("ADDR", hex(addr)\
                                    .replace("L", ""))\
                                    .replace("SIZE", hex(size)\
                                    .replace("L", ""))\
                                    .replace("VALUE", hex(value)\
                                    .replace("L", ""))
        data = Framework.firehose(d)
        if print_data:
            for x in data:
    ...
    def send_programmer():
        t = target.get()
        out = subprocess.Popen([SAHARA_SERVER, "-p", r"\\.\%s" % t.com,
                                "-s", "13:%s" % t.programmer_name,
                                "-b", "%s\\" % t.programmer_search_path],
    ...
    def firehosep(path):
        ...
        p = subprocess.Popen([FH_LOADER, r"--search_path=" + search,
