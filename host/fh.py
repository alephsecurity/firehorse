...
class Breakpoint:
    ...
    def pack(self):
        return struct.pack(self.FMT, self.bp_type,
                           self.addr, self.flag,
                           self.inst, self.size,
                           self.cb, self.msg)
...
class Firehorse:
    ...
    def pack(self):
        ...
        fh = struct.pack(self.FMT, mode, self.bpm.get_breakpoints_count(),
                         self.pm.get_patch_count(), 0, 0, 0)
        fh += self.bpm.pack()
        fh += self.pm.pack()
        fh += self.pc.pack()
        return fh
...
class Egg:
    ...
    def upload_egghunter(self):
        ...
        FH_FW.sendfile("../device/build/dload.payload", self.target.egghunter_base)
        Egg.uploaded = True
    ...
    def pack(self, partmask=0):
        ...
        for i in xrange(self.get_parts_count()):
            ...
            partdata = struct.pack(self.FMT,
                                   Egg.MAGIC1, i, len(part), self.dst+i*Egg.PART_SIZE, 0) + part
            cksum = self.cksum(partdata)
            partdata = partdata[:16] + struct.pack("<L", cksum)\
                       + partdata[20:] + struct.pack("<L", Egg.MAGIC2)
...
class XMLHunter:
    ...
    def upload_xmlhunter(self):
        ...
        FH_FW.sendfile("../device/build/xmlhunt.payload", self.target.egghunter_base)
    ...
    def do_send(self, dst, part):
        ...
        FH_FW.poke_payload(self.target.exe_addr,
                           self.target.arch / 8,
